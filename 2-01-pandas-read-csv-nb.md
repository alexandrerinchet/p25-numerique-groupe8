...
```{code-cell} ipython3
df[['Age', 'Fare']].describe() #doubles-crochets : on donne une liste de colonnes, donc renvoie une data frame (si simple crochet, renvoie pas une df) 
```
...
df.Age #pas cooooool, privilégier la notation suivante :
df['Age']
```
...
```{code-cell} ipython3
# votre code
pd.read_csv('data/petit-titanic.csv', sep=';', header=None, names=['PassengerId', 'Survived', 'Pclass', 'Name', 'Sex', 'Age', 'SibSp', 'Parch', 'Ticket', 'Fare', 'Cabin', 'Embarked'])
```
...
columns = ['Identifiant', 'Survécu', 'Pclass',
           'Nom', 'Genre', 'Age', 'SibSp', 'Parch',
           'Ticket', 'Tarif', 'Cabine', 'Embarquement']
pd.read_csv('data/petit-titanic.csv', sep=';', header=None, names=columns)
```
...
```{code-cell} ipython3
# votre code
df=pd.read_csv('data/petit-titanic.csv', sep=';', header=None, names=columns)
