...
# votre code
df4.dtypes
#les masses ne sont pas un float mais un objet
```
...
# votre code
df4['Mass (lb)'].unique()
#certaines valeurs ne sont pas renseignées (seulement une comparaison)
```
...
```{code-cell} ipython3
# votre code
df4['Mass (lb) orig']=df4['Mass (lb)'].copy()
df4['Mass (lb)'] = pd.to_numeric(df4['Mass (lb)'], errors='coerce')
df4['Mass (lb)']
df4['Mass (lb)'].isna().sum()
...
```{code-cell} ipython3
# votre code
df4['Mass (lb) orig']=df4['Mass (lb) orig'].str.replace('>','').str.replace('<','')
df4['Mass (lb) orig']=df4['Mass (lb) orig'].astype(dtype='int')
```
...
```{code-cell} ipython3
# votre code
df4['Mass (kg)']=(df4['Mass (lb) orig']/2.205).astype(dtype='int')
df4['Mass (kg)']
```
...
```{code-cell} ipython3
# votre code
df4['Mass (kg)'].sum() #192216kg
dfusa=df4[df4['Country'] == 'United States']
dfusa['Mass (kg)'].sum() #109604kg
```
...
```{code-cell} ipython3
# votre code
index_min = df4['Mass (kg)'].idxmin()
df4.loc[index_min, 'Country']
```
...
```{code-cell} ipython3
# votre code
mem = df4[df4['Artificial object'].str.contains('Memorial')]
mem #il s'agit du Luxembourg
```
