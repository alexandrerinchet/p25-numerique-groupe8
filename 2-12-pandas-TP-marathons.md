...
```{code-cell} ipython3
# votre code
df=pd.read_csv('data/marathon.txt', names=names, header=None, sep="\t")
df
```
...
```{code-cell} ipython3
# votre code
df.to_csv('marathon.csv', index=None)
```
...
```{code-cell} ipython3
# votre code
count_by_city = df.groupby(by='city').size()
```
...
```{code-cell} ipython3
# votre code
((df['seconds'] == df['duration'].dt.total_seconds()).sum()), df.shape
```
...
df['second'] = ((df['duration'] % one_hour) % one_minute) // one_second
```
