...
#j'ai dû copier data dans events car il ne trouvait pas le fichier sinon
events.head(10)
```
...
```{code-cell} ipython3
# your code
#on vérifie que la fin d'un événement n'est pas après le début du suivant
events['overlap'] = events['end'] > events['beg'].shift(-1)
events['overlap'].any()
#pas d'overlap
```
...
```{code-cell} ipython3
# your code
events['beg'].min(), events['end'].max(), events['end'].max() - events['beg'].min()
```
...
total_reservation_time = events['duration'].sum()

total_timespan = events['end'].max() - events['beg'].min()

usage_ratio = total_reservation_time / total_timespan

total_reservation_time, total_timespan, usage_ratio
```
...
```{code-cell} ipython3
# your code
monthly_duration = events.set_index('beg')['duration'].resample('M').sum()
weekly_duration = events.set_index('beg')['duration'].resample('W').sum()
yearly_duration = events.set_index('beg')['duration'].resample('Y').sum()

monthly_duration = events.groupby(events['beg'].dt.to_period('M'))['duration'].sum()
weekly_duration = events.groupby(events['beg'].dt.to_period('W'))['duration'].sum()
yearly_duration = events.groupby(events['beg'].dt.to_period('Y'))['duration'].sum()

monthly_duration.plot(kind='bar')
weekly_duration.plot(kind='bar') 
yearly_duration.plot(kind='bar')
```
...
def convert_timedelta_to_hours(timedelta: pd.Timedelta) -> int:
    return int(np.ceil(timedelta.total_seconds() / 3600))
```
...
```{code-cell} ipython3
# your code
num_regions = countries['region'].nunique()
countries_per_region = countries.groupby('region')['name'].nunique()

num_regions, countries_per_region
```
