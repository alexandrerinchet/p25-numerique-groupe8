...
```{code-cell} ipython3
# votre code
df_france.columns, len(df_france)
```
...
```{code-cell} ipython3
# votre code
df_italie = pd.DataFrame(by_country['Italy'])
df_italie['country'] = 'Italy'
df_fraita = pd.concat([df_france, df_italie], ignore_index=True)
df_fraita.head(5000)
```
...
for pays in by_country.keys() :
    df_paystemp = pd.DataFrame(by_country[pays])
    df_paystemp['country'] = pays
    df_pays.append(df_paystemp)
    
global_df = pd.concat(df_pays, ignore_index=True)

global_df.head(10000)
```
...
```{code-cell} ipython3
# votre code
mes_france_0101 = clean_df.loc[('France', '2021-01-01'), ['confirmed', 'deaths', 'recovered']]
mes_france_0101.head(100)
```
...
```{code-cell} ipython3
# votre code
mes_france = clean_df.loc['France']
mes_france.head(10)
```
...
```{code-cell} ipython3
# votre code
mes_france_italie = clean_df.loc[['France', 'Italy']]
mes_france_italie.head(5000)
```
...
```{code-cell} ipython3
# votre code
data_france = clean_df.loc['France'].reset_index() #je réinitialise l'index pour que la date
                                                   #soit à nouveau une colonne que je pourrai appeler comme axe des abcisses

data_france.plot(
    x='date',
    y=['confirmed', 'deaths', 'recovered'],
    title='Évolution des trois mesures en France',
    xlabel='Date',
...
data_france.plot(
    x='date',
    y=['confirmed', 'deaths'],
    title='Évolution des décès et cas confirmés en France',
    xlabel='Date',
    ylabel='Nombre',
)
plt.show()
